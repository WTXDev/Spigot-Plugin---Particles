							...
							if(stored.containsKey(player.getUniqueId()))
							{
								for (Player players : Bukkit.getOnlinePlayers())
								{
									...
									if(effect == Effect.MOBSPAWNER_FLAMES)
									{
										...
										{
											location = Bukkit.getPlayer(player.getUniqueId()).getEyeLocation().add(new Location(player.getWorld(), 0, 0.1, 0));
											players.playEffect(location, stored.get(player.getUniqueId()), 5);
										}
									else if (effect == Effect.HAPPY_VILLAGER)
									{
										location = Bukkit.getPlayer(player.getUniqueId()).getEyeLocation().add(new Location(player.getWorld(), 0, 0.5, 0));
				...
				case "set":
					...
					try {
						...
						case "flames": 
							...
							this.setPlayerParticles(effect, player); break;
						case "hearts": 
							effect = Effect.HEART;
							this.setPlayerParticles(effect, player); break;
						case "happyvillager": 
					...
					{
						ex.printStackTrace();
						this.sendMessageWithPrefix("Something happened. Try again and make sure you are choosing a valid particle.", player);
					}
